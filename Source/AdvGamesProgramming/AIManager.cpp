...
ANavigationNode* AAIManager::FindNearestNode(const FVector& Location)
{
	...
	return NearestNode;
	/*Basically, the way that the nodes are found is by comparing the current node with the nearest distance. 
	The problem with this however, is when there's two distances that are roughly similar, because of the imprecise nature of FVectors,
	there would be difficulty in figuring out which path to take. 
